...
const getSmartConfigFromEnv = (): Omit<SmartFhirClientOptions, 'request' | 'env' | 'expectedState' | 'pkceCodeVerifier'> => {
	...
	if (process.env.SMART_ISS) {
		if (!process.env.SMART_ISS.startsWith('https://')) {
			console.warn('SMART_ISS from environment variables is not HTTPS. Production environments require HTTPS.')
	...
	if (process.env.SMART_REDIRECT_URI) {
		if (!process.env.SMART_REDIRECT_URI.startsWith('https://')) {
			console.warn('SMART_REDIRECT_URI from environment variables is not HTTPS. Production environments require HTTPS.')
			// Depending on strictness, you might throw an error here in production.
		}
	...
	if (!config.iss && process.env.FHIR_BASE_URL) {
		if (!process.env.FHIR_BASE_URL.startsWith('https://')) {
			console.warn('FHIR_BASE_URL (used as ISS fallback) from environment variables is not HTTPS. Production environments require HTTPS.')
			// Depending on strictness, you might throw an error here in production.
		}
...
export const createSmartFhirClient = async (
	...
	if (!options.pkceCodeVerifier) {
		throw new Error('PKCE code_verifier must be provided in options for FHIR.oauth2.ready(). This should have been stored securely by the calling application.')
	}
	if (!options.expectedState) {
		throw new Error('Expected state must be provided in options for FHIR.oauth2.ready(). This should have been stored securely by the calling application.')
	}
	...
	try {
		...
		const errorDetails = error?.response?.data ? { data: error.response.data } : (error?.message ? {} : {rawError: error})
...
export const authorizeSmartClient = async (
	...
	const envConfig = getSmartConfigFromEnv(options.env) // options.env is for explicit env passing, otherwise process.env is used by getSmartConfigFromEnv
	...
	try {
		...
		if(!authorizationEndpoint.startsWith('https://')) {
			console.error(`Authorization endpoint ${authorizationEndpoint} from ${iss} is not HTTPS. This is a security risk.`)
